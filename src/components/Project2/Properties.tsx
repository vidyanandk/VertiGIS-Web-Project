                    ...
                    {model.selectedLayer ? (
                        <>
                            <Typography variant="h5" style={{ marginBottom: "1rem" }}>
                                {model.selectedLayer.name} - Properties
                            </Typography>
                            <Typography variant="body2" style={{ marginBottom: "1.5rem", lineHeight: 1.6 }}>
                                <strong>ID:</strong> {model.selectedLayer.id}<br />
                                <strong>Parent Layer ID:</strong> {model.selectedLayer.parentLayerId}<br />
                                <strong>Default Visibility:</strong> {model.selectedLayer.defaultVisibility ? "Yes" : "No"}<br />
                                <strong>Sub Layer IDs:</strong> {model.selectedLayer.subLayerIds || "None"}<br />
                                <strong>Min Scale:</strong> {model.selectedLayer.minScale}<br />
                                <strong>Max Scale:</strong> {model.selectedLayer.maxScale}<br />
                                <strong>Type:</strong> {model.selectedLayer.type}<br />
                                <strong>Geometry Type:</strong> {model.geo}<br />
                            </Typography>
                            <Button
                                onClick={() => {
                                    model.propertiesModel = false;
                                    model.selectedLayer = null;
                                }}
                                variant="outlined"
                                color="primary"
